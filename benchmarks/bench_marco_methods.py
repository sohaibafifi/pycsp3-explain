#!/usr/bin/env python3
"""
Benchmark MARCO enumeration variants on shared UNSAT cases.

Usage:
    uv run python benchmarks/bench_marco_methods.py
    uv run python benchmarks/bench_marco_methods.py --repeats 7 --warmup 1
    uv run python benchmarks/bench_marco_methods.py --verbose
...
from pycsp3_explain import marco, marco_core, marco_naive
...
def case_alldiff_collision() -> Tuple[List[Constraint], List[Constraint]]:
    # Use dom=range(5): AllDifferent is satisfiable alone; conflict comes from duplicates.
...
def case_large_irrelevant_pair_80() -> Tuple[List[Constraint], List[Constraint]]:
    """
    One tiny conflict embedded in a large soft set.
    """
    x = VarArray(size=40, dom=range(100))
    soft: List[Constraint] = [
        x[0] == 1,
        x[0] == 2,  # only true conflict pair
    ]
    for i in range(1, 40):
        soft.append(x[i] >= 0)
        soft.append(x[i] <= 99)
...
def case_two_conflicts_irrelevant_60() -> Tuple[List[Constraint], List[Constraint]]:
    """
    Two disjoint conflicts plus many irrelevant constraints.
    """
    x = VarArray(size=30, dom=range(100))
    soft: List[Constraint] = [
        x[0] == 1,
        x[0] == 2,  # conflict A
        x[1] == 3,
...
def case_alldiff_dense_core_24() -> Tuple[List[Constraint], List[Constraint]]:
    """
    Dense AllDifferent setting where core extraction can include many soft constraints.
    """
    n = 24
    x = VarArray(size=n, dom=range(n))
    soft: List[Constraint] = [AllDifferent(x)]
    for i in range(n - 1):
        soft.append(x[i] == i)
    soft.append(x[n - 1] == 0)
    return soft, []
...
def run_once(
    ...
    try:
        ...
        if method_name == "marco_core":
            if core_handoff > 0:
                method_kwargs["handoff_threshold"] = core_handoff
            method_kwargs["batch_base_ratio"] = core_base_ratio
            method_kwargs["sat_backoff_cap"] = core_backoff_cap

        pairs = list(
            method(soft, **method_kwargs)
        )
        ...
        if validate:
            valid_mus = all(is_mus(mus, hard=hard, solver=solver, verbose=method_verbose) for mus in muses)
            valid_mcs = all(is_mcs(mcs, soft, hard=hard, solver=solver, verbose=method_verbose) for mcs in mcses)
        else:
            valid_mus = True
            valid_mcs = True
...
def summarize(records: List[RunRecord]) -> List[SummaryRow]:
    ...
    for (case_name, method_name), recs in sorted(grouped.items()):
        valid_times = [r.elapsed_s for r in recs if r.success and r.valid_mus and r.valid_mcs]
        valid_mus_counts = [r.mus_count for r in recs if r.success and r.valid_mus and r.valid_mcs]
        valid_mcs_counts = [r.mcs_count for r in recs if r.success and r.valid_mus and r.valid_mcs]
        success_rate = sum(1 for r in recs if r.success) / len(recs)
        valid_rate = sum(1 for r in recs if r.success and r.valid_mus and r.valid_mcs) / len(recs)
        rows.append(
            SummaryRow(
                case=case_name,
                method=method_name,
...
def print_summary(rows: List[SummaryRow], baseline: str) -> None:
    ...
    for case_name in sorted(by_case):
        ...
        for row in sorted(
            ...
            print(
                f"{row.method:16} {median_txt} {p90_txt} "
                f"{row.success_rate:5.2f} {row.valid_rate:5.2f} {mus_txt:>4} {mcs_txt:>4}"
            )

    # Geometric mean speedup vs baseline.
    ...
    if speedups:
        ...
        for method_name in sorted(speedups):
            ...
            print(f"  {method_name:14} x{gmean:.3f} ({len(ratios)} cases)")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark MARCO methods")
    ...
    parser.add_argument("--max-iterations", type=int, default=1000, help="Forwarded to MARCO methods")
    parser.add_argument(
        "--core-handoff",
        type=int,
        default=-1,
        help="Optional handoff threshold for marco_core (<=0 means use algorithm default)",
    )
    parser.add_argument(
        "--core-base-ratio",
        type=int,
        default=2,
        help="Batch base ratio for marco_core (default: 2)",
    )
    parser.add_argument(
        "--core-backoff-cap",
        type=int,
        default=8,
        help="SAT backoff exponent cap for marco_core (default: 8)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Verbosity forwarded to MARCO methods and validators (default: -1)",
    )
